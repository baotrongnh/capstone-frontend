     ...
     return (
          <div className="flex-1 overflow-y-auto px-4 py-2">
               {messages.map((item) => (
                    ...
                    >
                         <div className={`max-w-[80%] px-3 py-2 rounded-lg ${item.sender === 'user' ? 'bg-primary text-white' : 'bg-gray-100 text-gray-800'}`}>
                              ...
                              {item.images?.map((src, i) => (
                                   <Image key={i} src={src} alt="attachment" width={400} height={200} unoptimized className="max-w-full rounded mt-1 max-h-48 object-contain" />
                              ))}
