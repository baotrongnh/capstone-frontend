...
export type ApartmentSearchQueryParams =
  paths["/api/v1/apartments/search"]["get"]["parameters"]["query"];
...
export type FurnishingType = NonNullable<
  ApartmentQueryParams["furnishingStatus"]
>;

export type OwnerApartmentListResponse =
  paths["/api/v1/apartments/owner/{ownerId}"]["get"]["responses"]["200"]["content"]["application/json"];

export type OwnerApartmentResponse = NonNullable<
  OwnerApartmentListResponse["data"]
>[number] & {
  owner?: {
    id?: string;
...
export type ApartmentRatingPayload =
  paths["/api/v1/apartments/{id}/rating"]["post"]["requestBody"]["content"]["application/json"];
export type ApartmentRatingResponse =
  paths["/api/v1/apartments/{id}/rating"]["post"]["responses"]["201"]["content"]["application/json"];

export type ApartmentOwnerResponse =
  paths["/api/v1/apartments/owner/{ownerId}"]["get"]["responses"]["200"]["content"]["application/json"];
...
export const apartmentService = {
  ...
  rating: async (id: string, payload: ApartmentRatingPayload) => {
    const { data } = await apiClient.post(
      `${endpoints.apartments}/${id}/rating`,
      payload,
    );
    return data.data;
  },

  cooperationContract: async (id: string | number) => {
    const { data } = await apiClient.get(
      `${endpoints.apartments}/${id}/cooperation-contract`,
    );
    return data.data;
  },
};
