import { ApartmentDetailResponse, ApartmentListResponse, ApartmentSearchQueryParams } from "@/types/apartment"
...
export const apartmentService = {
  ...
  create: async (apartmentData: string) => {
    const { data } = await apiClient.post(endpoints.apartments,
      apartmentData
    )
    return data
  },
  ...
  update: async (id: string | number, apartmentData: string) => {
    const { data } = await apiClient.put(`${endpoints.apartments}/${id}`,
      apartmentData
    )
    return data
  },
